/* Code Arduino pour la carte Educaduino (mesure de température et de pression) avec affichage sur écran LCD. Le code pour le câblage "standard" est fourni  avec les documents du stage*/
...
#define _NUMER0_BROCHE_ANALOGIQUE_TENSION A10  //Broche analogique utilisée par le capteur pour la mesure de la tension. (Dépend de la localisation du capteur sur la carte EDUCA DUINO Lab).
#define _NUMER0_BROCHE_ANALOGIQUE_COURANT A11 //Broche analogique utilisée par le capteur pour la mesure du courant. (Dépend de la localisation du capteur sur la carte EDUCADUINO Lab).
#define _NUMER0_BROCHE_ANALOGIQUE_TENSION2 A12  //Broche analogique utilisée par le capteur pour la mesure de la tension. (Dépend de la localisation du capteur sur la carte EDUCA DUINO Lab).
#define _NUMER0_BROCHE_ANALOGIQUE_COURANT2 A13 //Broche analogique utilisée par le capteur pour la mesure du courant. (Dépend de la localisation du capteur sur la carte EDUCADUINO Lab).long temps;
#define _NUMER0_BROCHE_ANALOGIQUE A8          //Broche analogique utilisée par le capteur pression. (Dépend de la localisation du capteur sur la carte EDUCADUINO Lab).
...
// La valeur de l'intensité du courant est déterminée par la mesure d'une tension aux bornes d'une résistance de 10kOhms
...
//Affichage des résultats
  ...
  // POUR DEUXIEME CAPTEUR DE TEMPERATURE Lecture de la valeur tension du capteur sur l'entrée analogique _NUMER0_BROCHE_ANALOGIQUE_TENSION.
  // La valeur mesurée par le Convertisseur Analogique Numérique prend pour valeur 0 pour une tension de 0V et 1023 pour une tension de 5V.
  int valeurNumeriqueTension2 = analogRead(_NUMER0_BROCHE_ANALOGIQUE_TENSION2);
  // Conversion de la valeur numerique en tension.
  float tension_T_V2   = ConvertiValeurMesureAnalogiqueEnTension(valeurNumeriqueTension2);

  // Lecture de la valeur courant du capteur sur l'entree analogique _NUMER0_BROCHE_ANALOGIQUE_TENSION2.
  // La valeur mesurée par le Convertisseur Analogique Numérique prend pour valeur 0 pour une tension de 0V et 1023 pour une tension de 5V.
  // Le courant est déterminé par la mesure de la tension aux bornes d'une résistance de 10kOhms.
  int valeurNumeriqueCourant2 = analogRead(_NUMER0_BROCHE_ANALOGIQUE_COURANT2);
  // Conversion de la valeur numérique en courant.
  float courant_A2   = ConvertiValeurMesureAnalogiqueEnCourant(valeurNumeriqueCourant2);

  //Calcul de la résistance
  float resistance_ohm2 = tension_T_V2 / courant_A2;
  ...
  //Appel de la méthode de la libraire pour la conversion de la résistance en température.
  float temperature_degreC2= ConvertiResistanceEnTemperature(resistance_ohm2);
//Affichage des résultats
  Serial.print("\t");
  Serial.print(" Temp : ");    // Transmission de la chaine " Température:"
  Serial.print(temperature_degreC2); // Transmission de la temperature en °C
  Serial.print(" degresC ");              // Transmission de l'unité.
  ...
  ecranLCD.setCursor(7,0);          // Positionne le curseur en 1ère colonne, 1ère ligne.
  ecranLCD.print("  ");        // 
  ecranLCD.print(temperature_degreC2);   // Affichage de la température.
  ecranLCD.print((char)223);            // Affichage du caractère '°'.
  ecranLCD.print("C");  
