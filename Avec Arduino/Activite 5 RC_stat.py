...
Modifié le 05/01/2021 par Jonas FORLOT pour acquisition sur carte Arduino avec le langage Python (grace a la bibliotheque nanpy)
...
from nanpy import SerialManager  # Utiliser par l'interpreteur python pour communiquer avec la carte Arduino
...
connection = SerialManager(device='COM10') # Windows: Le numéro de port est celui utilisé par la carte. Il est identifiable avec l'application "arduino"" par le menu [Outils][Port].Arduino




try:
    a = ArduinoApi(connection=connection)     # Création de l'objet d'exploitation la librairie arduino
except:
...
capa = [] # On créer 1 liste vide pour accueillir les données (N mesures de C)
