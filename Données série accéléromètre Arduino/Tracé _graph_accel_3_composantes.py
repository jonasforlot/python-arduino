...
liste_ax = [] # liste pour stocker les valeurs d'acceleration
liste_ay = [] # liste pour stocker les valeurs d'acceleration
liste_az = [] # liste pour stocker les valeurs d'acceleration
liste_t = []
...
def animate(i):
    ...
    if len(listeDonnees) == 4 : # parfois des lignes de données vides peuvent être envoyées, il faut les "écarter"
        accelx = (float(listeDonnees[1].decode()))/16834 # après consulation des données, nous choisissons le 2 ème élément de listeDonnees, on convertit l'accélération en g
        accely = (float(listeDonnees[2].decode()))/16834 # après consulation des données, nous choisissons le 3 ème élément de listeDonnees, on convertit l'accélération en g
        accelz = (float(listeDonnees[3].decode()))/16834 # après consulation des données, nous choisissons le 4 ème élément de listeDonnees, on convertit l'accélération en g
        ...
        while temps <= t_acquisition:
            liste_ax.append(accelx)
            print("ax = %f"%(accelx), " g") # affichage de la valeur de l'acceleration
            liste_ay.append(accely)
            print("ay = %f"%(accely), " g") # affichage de la valeur de l'acceleration
            liste_az.append(accelz)
            print("az = %f"%(accelz), " g") # affichage de la valeur de l'acceleration
            liste_t.append(temps)
            print("temps = %f"%(temps), " s") # affichage de la valeur du temps en partant de 0
            line0.set_data(liste_t,liste_ax)
            line1.set_data(liste_t,liste_ay)
            line2.set_data(liste_t,liste_az)
...
ax3.axis([0,t_acquisition,0,amax])
