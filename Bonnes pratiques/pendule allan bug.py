...
def animate(i):
     ...
     if len(listeDonnees) != 0 : # parfois des lignes de données vides peuvent être envoyées, il faut les "écarter"
         angle = float(listeDonnees[1].decode()) # après consulation des données, nous choisissons le 2ème élément de listeDonnees
         tempsmes = time.time()
         ...
         while tempsreel <= t_acquisition:
             ...
             print("angle = %f"%(angle), " degré") # affichage de la valeur d'angle
...
for i in range (len (liste_angle)):
     line = str(liste_temps[i]) +'\t'+ str(liste_angle[i])+'\n'
     lines.append(line)
 

fichier = open('data_arduino.odt', 'w')
