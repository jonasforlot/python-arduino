...
def animate(i):
    ...
    if len(listeDonnees) == 2 : # parfois des lignes de données vides peuvent être envoyées, il faut les "écarter"
        eclairement = (float(listeDonnees[1].decode())) # après consulation des données, nous choisissons le 4 ème élément de listeDonnees, on convertit l'accélération en g
